...
public final class ArticleFactory {
	...
	/**
	 * To allow modification of articles (and index page), always run createArticles
	 * again when opening the articles page
	 * 
	 */
	...
	public String getArticleIndex() {
		...
		do {
			...
			if (!article.getArticleGroupDescription()
					.equals(article.getPreviousArticle().getArticleGroupDescription())) {
				htmlArticleLine += "<h3 class=\".h3\">" + article.getArticleGroupDescription() + "</h3>";
			}

			htmlArticleLine += "<ul class=\"blog-ul\">";
			htmlArticleLine += "<li><a ";
			htmlArticleLine += "href=\"/articles/" + article.getArticleRestId() + "\">"
